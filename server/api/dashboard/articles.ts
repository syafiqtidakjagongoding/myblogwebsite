  ...
  if (method === "POST") {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (fileField && articleCodeField) {
        ...
        const publicPath = await uploadToMinio(buffer, objectName, "image/webp");
    ...
    if (!body.tags || body.tags.length === 0) {
      throw createError({ statusCode: 400, message: 'Please fill the article tags' })
    }
    ...
    const result = await db
      ...
      .values({
        ...
        path:
          body.path ||
          `/blog/${body.category}/${body.articleCode || Date.now()}`,
        tags: body.tags,
      })
      .returning();
  ...
  if (method === "PUT") {
    ...
    if (body.contentIDN !== undefined || body.contentEN !== undefined) {
      const existingContent = await db
        .select()
        .from(content)
        .where(eq(content.articleId, body.id!));

      if (existingContent.length > 0) {
        const contentUpdate: { contentIDN?: string; contentEN?: string } = {};
        if (body.contentIDN !== undefined)
          contentUpdate.contentIDN = body.contentIDN;
        if (body.contentEN !== undefined)
          contentUpdate.contentEN = body.contentEN;
        await db
          .update(content)
          .set(contentUpdate)
          .where(eq(content.articleId, body.id!));
